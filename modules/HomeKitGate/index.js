...
HomeKitGate.prototype.init = function (config) {
	...
	this.hk = new HomeKit(this.config.name, function(r) {
		if (r.method == "GET" && r.path == "/accessories") {
			...
			self.controller.addNotification("notification", "HomeKit PIN: " + this.config.pin, "module", "HomeKitGate");
		}
	}, this.config.pin, this.config.setupID || "", this.config.configNumber || 0, function() {
		self.config.pin = this.pin;
		self.config.setupID = this.setupID;
		self.config.configNumber = this.configNumber;
		self.config.qrUrl = this.qrUrl;
		console.logJS(arguments);
		self.saveConfig();
	});
	...
	this.controller.addNotification("notification", "HomeKit PIN: " + this.config.pin, "module", "HomeKitGate");
};
